...
- 각 섹션 최소 글자 수를 반드시 지킬 것 — 미달 시 재작성
...
0. 인트로      (최소 300자, ~70초)  : 강렬한 질문 훅 + 오늘 다룰 핵심 3가지 예고
1. 배경/현황    (최소 600자, ~130초) : 무슨 일이 일어났나? 맥락·수치·타임라인 상세 설명
2. 핵심 분석   (최소 800자, ~180초) : 왜 중요한가? 원인·구조·메커니즘 심층 해설, 전문가 시각
3. 영향과 전망  (최소 600자, ~130초) : 우리 생활/투자에 미치는 실질 영향 + 향후 시나리오 2~3가지
4. 마무리/CTA  (최소 300자, ~70초)  : 핵심 요약 3줄 + 시청자 실천 조언 + 구독·좋아요 유도

JSON 형식으로만 응답 (다른 텍스트 없이):

{
  "sections": [
    { "name": "인트로",     "script": "...(최소 300자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 70 },
    { "name": "배경",      "script": "...(최소 600자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 130 },
    { "name": "핵심분석",   "script": "...(최소 800자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 180 },
    { "name": "영향과전망", "script": "...(최소 600자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 130 },
    { "name": "마무리",    "script": "...(최소 300자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 70 }
  ]
}`;
