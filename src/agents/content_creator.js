...
async function generateLongVideoScript(item, competitorCtx = '') {
  ...
  const prompt = `당신은 유튜브 채널 "매일읽어주는남자"의 롱폼(7~10분) 대본 작가입니다.
아래 키워드로 총 420~600초(7~10분) 분량의 완성된 나레이션 대본을 작성하세요.
...
${competitorCtx}
━━━ 절대 규칙 ━━━
- script 필드는 실제 읽어야 할 전체 나레이션 문장을 빠짐없이 작성 (요약·개요 금지)
- 각 섹션 최소 글자 수를 반드시 지킬 것 — 미달이면 추가 내용을 덧붙여 채울 것
- 한 문장 최대 30자, 구어체, TTS 친화적
- 숫자는 한글 발음으로: "14.9퍼센트", "팔천억원"
- 각 섹션 끝에는 반드시 다음 섹션으로 자연스럽게 넘어가는 연결 문장 1개 추가
- 추상 표현 금지: 반드시 구체 수치/사례/사건으로 뒷받침

━━━ 섹션 구성 (각 섹션별 최소 글자 수 필수) ━━━
0. 인트로      (최소 350자, ~70초)
   - 첫 문장: 충격적 수치 또는 반직관적 사실 1문장으로 바로 시작 (자기소개·"안녕하세요" 금지)
   - 오늘 다룰 핵심 포인트 3가지 예고
   - 마지막 문장: 끝까지 봐야 하는 이유 1문장

1. 배경/현황    (최소 650자, ~130초)
   - 무슨 일이 일어났나? 맥락·수치·타임라인 상세 설명
   - 마지막 문장: "그렇다면 왜 이런 일이 생겼을까요?" 형식의 다음 섹션 연결

2. 핵심분석    (최소 900자, ~180초)
   - 왜 중요한가? 원인·구조·메커니즘 심층 해설
   - 전문가 시각, 구체적 수치 3개 이상 포함
   - 마지막 문장: "이 상황이 우리에게 어떤 영향을 줄까요?" 형식의 연결

3. 영향과전망  (최소 650자, ~130초)
   - 우리 생활/투자에 미치는 실질 영향 설명
   - 향후 시나리오 2가지 (낙관·비관)
   - ★ 마지막 3문장: 시청자가 지금 당장 할 수 있는 구체적 행동 2가지를 명시
     예) "그렇다면 지금 우리는 뭘 해야 할까요? 첫째, ○○하세요. 둘째, ○○를 피하세요."
   - 절대 "각자의 판단이 필요합니다" 같은 빈말로 끝내지 말 것

4. 마무리/CTA  (최소 350자, ~70초)
   - 오늘 핵심 3줄 요약 (영향과전망의 행동 지침 재강조)
   - 다음에 볼 관련 주제 1개 예고: "다음 영상에서는 ○○에 대해 다룰 예정이니 알림 설정 해두세요"
   - 구독·좋아요·알림 설정 자연스럽게 유도
   - 마지막 문장: "오늘도 함께해 주셔서 감사합니다, 매읽남이었습니다."

JSON 형식으로만 응답 (다른 텍스트 없이):

{
  "sections": [
    { "name": "인트로",     "script": "...(최소 350자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 70 },
    { "name": "배경",      "script": "...(최소 650자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 130 },
    { "name": "핵심분석",   "script": "...(최소 900자 이상 실제 나레이션)...", "key_point": "핵심 한 줄", "duration_seconds": 180 },
    { "name": "영향과전망", "script": "...(최소 650자 이상 실제 나레이션, 마지막에 구체적 행동 2가지 필수)...", "key_point": "핵심 한 줄", "duration_seconds": 130 },
    { "name": "마무리",    "script": "...(최소 350자 이상, 다음 영상 예고 + 매읽남이었습니다로 마무리)...", "key_point": "핵심 한 줄", "duration_seconds": 70 }
  ]
}`;
  ...
  // 섹션별 최소 글자 수 검증
  const minChars = { 인트로: 350, 배경: 650, 핵심분석: 900, 영향과전망: 650, 마무리: 350 };
  for (const section of result.sections ?? []) {
    const min = minChars[section.name];
    const len = (section.script ?? '').length;
    if (min && len < min) {
      logger.warn(`[content_creator] 섹션 "${section.name}" 글자 수 미달: ${len}자 < ${min}자 (${item.keyword})`);
    }
  }
