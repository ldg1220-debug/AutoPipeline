      ...
      'https://api.openai.com/v1/chat/completions',
      {
        ...
        messages: [{
          ...
          content:
            ...
            `[콘텐츠 브리프]\n${brief}\n\n` +
            `[생성된 대본]\n${script}\n\n` +
            `아래 기준으로 0~100점 채점. 하나라도 치명적 결함 시 50점 이하:\n` +
            `- 훅이 브리프 각도와 일치하고 12자 이내 ?/!로 끝나는가 (25점)\n` +
            `- context: 무슨 일→왜→나에게 영향 순서가 지켜지는가 (20점)\n` +
            `- 수치에 반드시 기준 명시: ✗"5만원 증가" ✓"1억 기준 5만원 증가" (20점)\n` +
            `- insight: 원인→중간과정→결과→행동 인과관계 완결, 비약 없는가 (20점)\n` +
            `- CTA가 구독/좋아요/저장을 유도하는가 (15점)\n\n` +
            `JSON 반환: {"score":85,"pass":true,"feedback":"어떤 부분을 어떻게 고쳐야 하는지 구체적 지시"}`,
        }],
